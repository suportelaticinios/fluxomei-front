...
const url = "http://192.168.2.2:8082/api-fluxomei/entrada/";
...
{
    // faz uma requisição a api para gerar relatório mensal
    fetch('http://192.168.2.2:8082/api-fluxomei/conta/listar', {
        method: 'GET',
        headers: {
            "Authorization": `Bearer ${token}` // token no header
        },
...
{
    // faz uma requisição a api para gerar relatório mensal
    fetch('http://192.168.2.2:8082/api-fluxomei/categoria/listar', {
        method: 'GET',
        headers: {
            "Authorization": `Bearer ${token}` // token no header
        },
...
function montarTabela (dados)
{
    ...
    for (let d = 0; d < dados.length; d++)
    {
        tbEntradas.innerHTML += '<tr class="hover:bg-gray-50">'+
                                ...
                                '<td class="px-4 py-2">'+ dados[d].CONTA +'</td>'+
